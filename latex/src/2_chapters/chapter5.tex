...
\begin{figure}[ht]
    ...
    \begin{subfigure}{0.8\textwidth}
        ...
        \caption{Representation of a sinusoidal function $y(x) = a \; \sin(x) + b \; \cos(x)$. Example of there are, for instance, the position of a wirte with respect to time in a harmonic movement $x(t) = A\cos(\omega t + \phi).$}
...
\begin{figure}[ht]
    ...
    \begin{subfigure}{0.8\textwidth}
        ...
        \caption{Representation of a sinusoidal function $y(x) = a \; \sin(x) + b \; \cos(x) \pm \varepsilon$. Example of there are, for instance, the position of a write with respect to time in a harmonic movement $x(t) = A\cos(\omega t + \phi).$}
...
A final note, back to correlation. The Pearson sample correlation coefficient $r$, introduced by Karl Pearson in 1895, measures the strength and direction of linear association between two variables, taking values in the interval $[-1,1]$. In simple linear regression, people sometimes confuse that with its square $r^2$, normally called \textit{linear correlation coefficient}. 

\medskip

This measure, bounded between \(0\) and \(1\), was later interpreted by Sewall Wright in 1921 as the coefficient of determination, representing the proportion of variance in the response variable explained by the linear relationship with the predictor \cite{wright_1921_correlation}. However, a high $r^2$ does not imply causation and may be misleading if the underlying relationship is non-linear, influenced by outliers, or driven by omitted variables. The frequent confusion between $r$ and $r^2$ arises because both quantify aspects of linear association and coincide in sign and magnitude only in special cases, while $r^2$, being always non-negative, is often reported as a measure of explanatory power rather than direction.
