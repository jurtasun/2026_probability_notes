...
\]
which is defined on the interval $[0,1]$ and is therefore well-suited for modeling probabilities. 

\medskip

Its parameters $\alpha$ and $\beta$ can be interpreted as prior pseudo-counts, with $\alpha$ representing prior evidence for heads and $\beta$ for tails; in general, $\alpha$ and $\beta$ act as effective sample sizes. More precisely, $\alpha-1$ and $\beta-1$ can be interpreted as \textit{prior counts only under a particular parametrization}; in general, $\alpha$ and $\beta$ act as effective sample sizes.
...
Since the binomial coefficient does not depend on $\theta$, we may write
\[
...
\[
    \mathbb{E}[\theta \mid x] = \frac{\alpha + k}{\alpha + \beta + n},
\]
which can be interpreted as a weighted average between prior information and observed data.
