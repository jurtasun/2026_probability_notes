...
In statistical physics, the Ising model describes a lattice of spins, each taking values $\pm 1$, interacting with their neighbors. The probability of a configuration $\mathbf{x}$ is the Boltzmann distribution $\pi(\mathbf{x}) \propto \exp(-E(\mathbf{x})/T)$, where $E(\mathbf{x})$ is the energy and $T$ is temperature. Direct sampling is infeasible for large lattices, but the Glauber dynamics---a Gibbs sampler for the Ising model---provides an efficient Markov chain whose stationary distribution is exactly $\pi$ \cite{glauber_1963_ising}. This connection between MCMC and physical simulation runs deep: thermodynamics and Bayesian inference share, at a mathematical level, the same fundamental structure of sampling from distributions proportional to exponentials.
...
MCMC methods raise practical questions that classical Monte Carlo does not. Because consecutive states of the chain are correlated, the effective sample size is smaller than the number of iterations. Assessing whether the chain has converged to stationarity---whether the burn-in period has been long enough---is a non-trivial diagnostic problem. A widely used approach is to run multiple chains from overdispersed starting points and check whether they have mixed together, a criterion formalized by Gelman and Rubin \cite{gelman_1992_inference}. Despite these complications, MCMC has become a cornerstone of modern probabilistic inference, enabling statistical models of a complexity that analytic methods could not reach.
...
The sequential nature of language has recently prompted renewed interest in alternatives to the transformer that preserve explicit state representations. \textit{State space models} (SSMs), and in particular the Mamba architecture \cite{gu_2023_mamba}, revisit the idea of a hidden state that is updated sequentially, but formulate it in continuous-time terms borrowed from control theory and signal processing. The state $h_t$ is updated by a linear recurrence
\begin{equation}
    h_t = \bar{A} h_{t-1} + \bar{B} x_t \; , \qquad y_t = C h_t \; ,
\end{equation}
where $\bar{A}$ and $\bar{B}$ are discretizations of continuous-time system matrices, and the key innovation is making the parameters of this recurrence \textit{input-dependent}---different parts of the sequence receive different dynamics. The result is an architecture that combines the efficiency of linear recurrence with the selectivity of the attention mechanism, and that explicitly encodes the idea that the future depends on the present state rather than on the raw history. The loop from Markov's memoryless property to the latest sequence modeling architectures has, in a sense, come full circle: the most principled way to model long sequences may still be to ask, carefully and thoughtfully, what information the present state must contain to render the past irrelevant.
...
The arc traced in this chapter spans more than a century, from a theological dispute in late Imperial Russia to the neural networks that process billions of documents daily. What is perhaps most striking about this arc is not the sophistication of the destination, but the continuity of the ideas throughout. The Markov property---the compression of history into a sufficient present state---the transition matrix---states as vectors, dynamics as linear maps---the stationary distribution---the long-run behavior that emerges from iterated transitions---and the ergodic theorem---the convergence of time averages to ensemble averages---all of these ideas, developed in the first decade of the $20^{\text{th}}$ century for the analysis of poetry, recur as structural principles in the most advanced systems of the $21^{\text{st}}$. Mathematics, when it is right, does not become obsolete; it finds new homes.
